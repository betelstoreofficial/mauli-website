...
const PROOF = [
  ...
  { k: '18', label: 'Natural ingredients', body: 'Gulkand, dry fruits, rose, saffron and 14 others - nothing unhealthy hiding inside.' },
      ...
      <div className="mx-auto max-w-[1480px] px-6 md:px-10">
        <div className="mb-14 max-w-[44ch]">
          ...
          <RevealHeading
            ...
            className="text-display max-w-[16ch] text-[clamp(2rem,4.6vw,3.6rem)] font-bold leading-[1.06] text-cream"
          />
          <Reveal delay={0.1}>
            <p className="mt-5 max-w-[52ch] text-[1rem] font-light leading-relaxed text-cream/65">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {PROOF.map((p, i) => (
              <motion.div
                ...
                className="rounded-2xl border border-cream/10 bg-forest-900/40 p-6"
              >
                <div className="text-display text-4xl font-bold gold-text">{p.k}</div>
                <div className="text-display mt-2 text-lg font-bold text-cream">{p.label}</div>
                <p className="mt-1.5 text-[0.8rem] font-light leading-relaxed text-cream/55">{p.body}</p>
              </motion.div>
            ))}
          </div>
        </div>
